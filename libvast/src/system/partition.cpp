...
pack(flatbuffers::FlatBufferBuilder& builder, const active_partition_state& x) {
  ...
  // TODO: When we get more than two options here (in particular stores from user
  // plugins) we need to implement some kind of protocol where the partition can
  // request the chunk that should be placed into `store_header.data`.
  flatbuffers::Offset<flatbuffers::String> store_name;
  flatbuffers::Offset<flatbuffers::Vector<uint8_t>> store_data;
  if (x.partition_local_stores) {
    // FIXME: pull out strings into named constants
    store_name = builder.CreateString("local");
    auto data = fbs::serialize_bytes(builder, store_path_for_partition(x.id));
    if (!data)
      return data.error();
    store_data = *data;
  } else {
    store_name = builder.CreateString("global");
  }
      ...
      [=](chunk_ptr chunk) {
        ...
        switch (self->state.store_type) {
          ...
          default:
            ...
            VAST_ASSERT(false, "encountered unhandled store type");
        }
        if (!self->state.store) {
